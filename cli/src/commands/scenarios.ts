...
// 2. `catalog`: print the public scenario-family catalog.
// 3. `--validate <path>`: one-tick-boot harness for fixture scenarios
// generated by the merged `riptide-config` skill. Loads the scenario
...
import { createScenariosCatalogCommand } from "./scenarios-catalog.js";
import {
  validateScenario,
  type SpawnEngine,
...
export function createScenariosCommand(
  ...
  const command = new Command("scenarios")
    .description(
      "List built-in scenario presets; print the catalog; with --validate, boot-test a fixture scenario directory generated by riptide-config"
    )
    .option(
      "--validate <path>",
    ...
    .addHelpText(
      "after",
      "\nCatalog source: fixtures/scenarios/catalog.toml\n"
    );

  command.addCommand(
    createScenariosCatalogCommand({ repoRoot: deps.monorepoRoot })
  );
