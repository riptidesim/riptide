...
async function defaultRunOne(ctx: RunOneContext): Promise<RunOneResult> {
  ...
  // riptide-config skill may author a `policies.json` next to their
  // `run-config.json`. Configured user repos normally keep personas
  // inline in the adapter instead. When a fixture sidecar
