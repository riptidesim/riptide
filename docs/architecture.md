...
3. **Personas** — user repos declare personas inline in the adapter's `[personas.*]` tables. The `fixtures/personas/` libraries are monorepo fixture assets for intentionally authored fixture-mode scenarios; they are not copied to `.riptide/personas/`.
...
6. **Failure-mode taxonomy** — categories like `whale_concentration`, `margin_cascade_from_oracle_shock`, `price_manipulation_via_swap`, `impermanent_loss_spike`. The `riptide-config` skill uses this taxonomy after adapter lint and harness smoke pass.
