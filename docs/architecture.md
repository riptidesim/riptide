...
3. **Personas** — user repos declare personas inline in the adapter's `[personas.*]` tables. The older `fixtures/personas/` libraries are monorepo fixture assets the scenarios skill can consult when intentionally writing fixture-mode scenarios; they are not copied to `.riptide/personas/`.
4. **Scenarios** — engine shocks (oracle trajectories, scheduled actions) mounted from declarative presets. Replay scenarios additionally read `initial-state.json`, `trajectory.json`, and `oracle-trajectory.json`, with an optional replay-scoped adapter override when the recorded program shape diverges from the one your repo currently builds. See `fixtures/scenarios/` and `engine/src/scenario/preset_spec.rs`.
5. **Parameters** — run-config knobs that sweep over the dimensions that matter: whale share, shock magnitude, trade size, leverage, depositor concentration.
6. **Failure-mode taxonomy** — categories like `whale_concentration`, `margin_cascade_from_oracle_shock`, `price_manipulation_via_swap`, `impermanent_loss_spike`. The `riptide-scenarios` skill matches your adapter's shape against this taxonomy.
7. **Invariants** — machine-checkable properties (`no_bad_debt`, `reserve_a > 0`, `k == reserve_a * reserve_b` within tolerance) declared inline in the adapter. The engine exits non-zero when any invariant fires, so invariants double as CI gates. `riptide init` now ships starter invariants per protocol class, with live lending checks and commented templates for classes that still need adapter observations.
...
- **`riptide adapt --adapter <toml>`** is the adapter-only smoke with a lint preflight: when the adapter's lineage source is machine-checkable, adapt runs lint first and aborts before engine spawn on a concrete fail. Harness-required user repos should prefer `riptide run --adapter <toml> --harness .riptide/harness --seeds 1 --seed-root 1337` as the first end-to-end smoke, because it exercises the same setup path the real scenario battery will use.
