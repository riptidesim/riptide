...
> **Economic semantics status.** `lending.v1` semantics are the first implemented class. Other class strings (`perps-margin.v1`, `amm.v1`, `lst.v1`, `stablecoin.v1`) remain design targets. AMM currently uses `protocol = "generic"` and Riptide's generic SBF/IDL runtime; `amm.v1` semantics is future work.
