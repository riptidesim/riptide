...
fn validate_adapter_for_lending(adapter: &Adapter) -> Result<()> {
    for (ix_name, expected_action) in EXPECTED_INSTRUCTIONS {
        let mapping = adapter.instructions.get(*ix_name).ok_or_else(|| {
            anyhow!(
                "lending primitive: adapter [instructions] missing required key `{}`",
                ix_name
            )
        })?;
    ...
    for (path, expected_obs) in EXPECTED_STATE_MAPPING {
        let obs = adapter.state_mapping.get(*path).ok_or_else(|| {
            anyhow!(
                "lending primitive: adapter [state_mapping] missing required key `{}`",
                path
            )
        })?;
        if obs != expected_obs {
            return Err(anyhow!(
                "lending primitive: adapter [state_mapping].`{}` must be `{}`, got `{}`",
                path,
                expected_obs,
                obs
            ));
...
impl LiteSvmHarness {
    ...
    pub fn bootstrap(config: LiteSvmBootstrapConfig) -> Result<Self> {
        ...
        if let Some(adapter) = config.adapter.as_ref() {
            validate_adapter_for_lending(adapter)
        ...
        // oracles (e.g. the shipped lending.toml) keeps
