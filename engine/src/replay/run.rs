...
use crate::{
    adapter::{Adapter, ArgLiteral},
    agent::state::Agent,
    primitive::{Primitive, PrimitiveError},
    replay::{
    ...
    sim::run::{
        build_invariants_summary, build_summary, record_tick_snapshot, SimulationAbort,
    },
    types::{
        Policy, PositionSizing, PositionSizingStrategy, SimulationResult, SimEvent, SimOutcome,
        Trigger, RunConfig, InvariantViolation,
    },
};
...
fn collect_instruction_actor_ids(
    ...
    if instruction.agent.trim().is_empty() {
        bail!("replay instruction `{}` missing non-empty `agent`", instruction.name);
    }
    ...
    H: Primitive + ?Sized,
{
    ...
    let mut agents: Vec<Agent> = bundle
        .actor_ids
        .iter()
        .map(|actor| Agent::new(actor.clone(), synthetic_replay_policy(actor), 0.0).with_starting_price(bundle.starting_price))
        .collect();
    ...
    let agent_finals: Vec<_> = agents.iter().map(|agent| agent.final_state(final_price)).collect();
...
mod tests {
    ...
    fn collect_actor_ids_passes_target_through_when_adapter_declares_it() {
        ...
        let instruction = make_instruction(
            "swap",
            "trader-0",
            Some(ArgLiteral::String("GLe1xiYPUBRQCrxQA1wMVSESmCqY1YgVi1Rrhu7oMUhk".into())),
        );
        let mut actor_ids = BTreeSet::new();
        collect_instruction_actor_ids(&instruction, &adapter, &mut actor_ids).unwrap();
        ...
        assert!(
            !actor_ids
                .contains("GLe1xiYPUBRQCrxQA1wMVSESmCqY1YgVi1Rrhu7oMUhk"),
    ...
    fn collect_actor_ids_rejects_empty_target_only_for_pairwise() {
        ...
        let mut actor_ids = BTreeSet::new();
        let err = collect_instruction_actor_ids(&instruction, &adapter, &mut actor_ids)
            .unwrap_err();
