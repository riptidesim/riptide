...
//!
//! End-to-end coverage that the shipped `perpetuals.so` loads in
...
//!
//! Three tests:
//! 1. `perpetuals_roundtrip_long_path` — init → deposit → open (long)
//! → close on flat oracle → withdraw. Asserts PnL is ~0, collateral
//! round-trips, OI returns to zero.
//! 2. `perpetuals_liquidation_cascade` — init → deposit → open (long, 5x)
//! → oracle shock → liquidate → inspect cumulative_socialized_loss.
//! 3. `perpetuals_is_deterministic` — two fresh instances replay the
...
// --- Byte-level constants mirrored from `programs/perpetuals/src/state.rs` ---
...
fn perps_so_path() -> PathBuf {
    workspace_root().join("programs/perpetuals/target/deploy/perpetuals.so")
}
...
fn skip_if_missing(paths: &[&Path]) -> bool {
    for p in paths {
        if !p.exists() {
            eprintln!(
                "skipping t17 perpetuals LiteSVM test: {} missing \
                 (build both with \
                 `cargo build-sbf --manifest-path programs/perpetuals/Cargo.toml` and \
...
#[test]
fn perpetuals_roundtrip_long_path() {
...
#[test]
fn perpetuals_liquidation_cascade() {
    if skip_if_missing(&[&perps_so_path(), &oracle_so_path()]) {
        return;
    }
    let mut h = PerpsHarness::new();
...
#[test]
fn perpetuals_is_deterministic() {
    if skip_if_missing(&[&perps_so_path(), &oracle_so_path()]) {
        return;
    }
...
#[test]
fn perpetuals_rejects_leverage_over_cap() {
    if skip_if_missing(&[&perps_so_path(), &oracle_so_path()]) {
        return;
    }
    let mut h = PerpsHarness::new();
...
#[test]
fn perpetuals_rejects_cross_market_liquidation() {
    if skip_if_missing(&[&perps_so_path(), &oracle_so_path()]) {
        return;
    }
    let mut h = PerpsHarness::new();
