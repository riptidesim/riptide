...
//!    `svm_mut()` accessor. The perpetuals program reads the oracle
...
// ---------------------------------------------------------------------------
// Local Borsh mirrors of perpetuals's instruction + account surfaces.
...
fn perpetuals_idl() -> PathBuf {
    workspace_root().join("fixtures/idls/perpetuals.json")
}
...
///   actually exercising perpetuals + admin_mock_oracle against the
...
fn perps_proof_adapter_toml(owner_clause: &str) -> String {
    let so = perpetuals_so().display().to_string();
    let idl = perpetuals_idl().display().to_string();
    format!(
        r#"
protocol = "generic"
program_so = "{so}"
...
fn bootstrap_harness(name: &str) -> GenericHarness {
    ...
    GenericHarness::bootstrap(GenericBootstrapConfig {
        program_so: perpetuals_so(),
        idl_path: perpetuals_idl(),
        agent_count: 1,
        adapter,
    })
