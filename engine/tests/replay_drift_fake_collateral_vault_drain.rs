//! Drift-shape fake-collateral vault-drain replay gate.
//!
...
//! borrowable value. Drift runs on Solana, but this pack does NOT
//! replay Drift's mainnet bytecode, admin-control surface, oracle
//! accounts, or slot state — it reproduces the ECONOMIC GEOMETRY on
//! Riptide's Solend-fork toy lending program. The oracle trajectory
//! realizes the accepted-collateral mark and collapse as declared
//! ticks; the resulting underwater position lands real bad debt at
//! liquidation.
//!
//! This test locks four things:
//!
//! 1. The replay runs end-to-end on the replay-scoped adapter
//!    `fixtures/replays/drift-fake-collateral-vault-drain/adapter.toml`
//!    — a copy of the shipped `lending.toml` with a
//!    `collateral_backing` invariant added. The shipped adapter stays
//!    clean so the hero-grid hash and the existing incident replay
//!    hashes are preserved.
//! 2. The result is deterministic under a fixed mark-borrow-collapse-
...
//! 4. The declared `collateral_backing` invariant fires at the cascade
//!    tick — the tick at which the liquidator settles the underwater
//!    attacker and the protocol realizes the shortfall — and only
//!    there.
...
fn fixture_dir() -> PathBuf {
    monorepo_root()
        .join("fixtures")
        .join("replays")
        .join("drift-fake-collateral-vault-drain")
}
...
fn run_fixture() -> SimulationResult {
    ...
    let adapter = load_adapter(&fixture.join("adapter.toml"))
        .expect("load replay-scoped Drift-shape lending adapter");
...
#[test]
fn drift_fake_collateral_vault_drain_matches_expected_summary_and_is_deterministic() {
    let fixture = fixture_dir();
    ...
    assert_eq!(
        first, second,
        "Drift fake-collateral replay diverged across back-to-back runs",
    );

    let actual_hash = canonical_hash(&first);
    assert_eq!(
        actual_hash, expected.result_sha256,
        "Drift fake-collateral replay hash drifted; update fixtures/replays/drift-fake-collateral-vault-drain/expected-summary.json if the new output is intentional",
    );

    assert_eq!(first.events.len(), expected.event_count);
    assert_eq!(
        first.run_config.scenario,
        "replay:drift-fake-collateral-vault-drain"
    );
    ...
    assert_eq!(
        invariant_rows.len(),
        1,
        "replay adapter declares exactly one invariant (collateral_backing)"
    );
    let collateral_backing = &invariant_rows[0];
    assert_eq!(collateral_backing["name"], "collateral_backing");
    assert_eq!(collateral_backing["field"], "bad_debt");
    assert_eq!(collateral_backing["op"], "==");
    assert_eq!(
        collateral_backing["firings"]
            .as_u64()
            .expect("firings is a number") as usize,
        expected.bad_debt_invariant_firings,
        "collateral_backing invariant firing count drifted",
    );
    assert!(
    ...
    let first_invariant_event_tick = first
        ...
        .iter()
        .find_map(|event| {
            if event.action == "invariant_violation:collateral_backing" {
                Some(event.tick)
            } else {
                None
            }
        })
        .expect("expected at least one invariant_violation:collateral_backing event");
    assert_eq!(
        first_invariant_event_tick, expected.terminal_bad_debt_tick,
        "collateral_backing invariant fired at a different tick than the cascade",
    );
}
