//! Loopscale-shape collateral-mispricing bad-debt replay gate.
//!
...
//! Loopscale's mainnet bytecode or slot state — it reproduces the
//! ECONOMIC GEOMETRY on Riptide's Solend-fork toy lending program. The
//! oracle-trajectory realizes the mispricing as a single-tick
//! correction; the resulting underwater position lands real bad debt
//! at liquidation.
//!
//! This test locks four things:
//!
//! 1. The replay runs end-to-end on the replay-scoped adapter
//!    `fixtures/replays/loopscale-collateral-mispricing/adapter.toml` —
//!    a copy of the shipped `lending.toml` with a `collateral_health`
//!    invariant added (the slide-08-promised name, bound to the
//!    primitive's `bad_debt` snapshot field). The shipped adapter stays
//!    clean so the hero-grid hash and the lending-whale-bad-debt
//!    replay's canonical hash are preserved.
//! 2. The result is deterministic under a fixed mispricing-then-
...
//! 4. The declared `collateral_health` invariant fires at the cascade
//!    tick — the tick at which the liquidator settles the underwater
//!    borrower and the protocol realizes the shortfall — and only
//!    there.
...
fn fixture_dir() -> PathBuf {
    monorepo_root()
        .join("fixtures")
        .join("replays")
        .join("loopscale-collateral-mispricing")
}
...
fn run_fixture() -> SimulationResult {
    ...
    let adapter = load_adapter(&fixture.join("adapter.toml"))
        .expect("load replay-scoped Loopscale-shape lending adapter");
...
#[test]
fn loopscale_collateral_mispricing_matches_expected_summary_and_is_deterministic() {
    let fixture = fixture_dir();
    ...
    assert_eq!(
        first, second,
        "Loopscale collateral-mispricing replay diverged across back-to-back runs",
    );

    let actual_hash = canonical_hash(&first);
    assert_eq!(
        actual_hash, expected.result_sha256,
        "Loopscale collateral-mispricing replay hash drifted; update fixtures/replays/loopscale-collateral-mispricing/expected-summary.json if the new output is intentional",
    );

    assert_eq!(first.events.len(), expected.event_count);
    assert_eq!(
        first.run_config.scenario,
        "replay:loopscale-collateral-mispricing"
    );
    ...
    assert_eq!(
        invariant_rows.len(),
        1,
        "replay adapter declares exactly one invariant (collateral_health)"
    );
    let collateral_health = &invariant_rows[0];
    assert_eq!(collateral_health["name"], "collateral_health");
    assert_eq!(collateral_health["field"], "bad_debt");
    assert_eq!(collateral_health["op"], "==");
    assert_eq!(
        collateral_health["firings"]
            .as_u64()
            .expect("firings is a number") as usize,
        expected.bad_debt_invariant_firings,
        "collateral_health invariant firing count drifted",
    );
    assert!(
    ...
    let first_invariant_event_tick = first
        ...
        .iter()
        .find_map(|event| {
            if event.action == "invariant_violation:collateral_health" {
                Some(event.tick)
            } else {
                None
            }
        })
        .expect("expected at least one invariant_violation:collateral_health event");
    assert_eq!(
        first_invariant_event_tick, expected.terminal_bad_debt_tick,
        "collateral_health invariant fired at a different tick than the cascade",
    );
}
