//! UXD-style stablecoin collateral-cascade + redemption-run replay gate.
//!
//! Locks five things on the `stablecoin-uxd-style-collateral-cascade`
...
//! 4. `no_hedge_loss_during_healthy_run` fires at tick 3 (the
//!    scheduled `apply_hedge_loss`) and persists through the
//!    terminal tick.
//! 5. `full_backing` fires at tick 3 (ratio drops below 10_000 bps as
//!    a direct consequence of the haircut against fully-initialized
//!    pool state) and `no_redemption_queue_formation` fires at
//!    tick 4 (the panic cohort overruns the reserve buffer).
//!
//! This is the credibility claim the spec asks for: a named
//! UXD-style collateral-cascade / redemption-pressure proof where at
//! least one machine-checkable invariant fires at a named tick, with
//! a stable rerun command and artifact set. The proof is
//! single-program on purpose — it does NOT model a real hedge-venue
//! integration. Historical inspiration for the backing-gap +
//! redemption-pressure geometry: the November 2022 UXD delta-neutral
//! backing gap after the Mango exploit wiped the hedge leg.
...
fn fixture_dir() -> PathBuf {
    monorepo_root()
        .join("fixtures")
        .join("replays")
        .join("stablecoin-uxd-style-collateral-cascade")
}
...
#[test]
fn stablecoin_uxd_style_collateral_cascade_matches_expected_and_is_deterministic() {
    // Artifact gate runs up front: CI hard-fails on a missing SBF
    // artifact (`skip_if_missing` panics when `CI` is set), local dev
    // soft-skips by early-returning from the test body.
    ...
    assert_eq!(
        first, second,
        "stablecoin UXD-style collateral-cascade replay diverged across back-to-back runs",
    );
    ...
    assert_eq!(
        ...
        "replay:stablecoin-uxd-style-collateral-cascade"
    );
