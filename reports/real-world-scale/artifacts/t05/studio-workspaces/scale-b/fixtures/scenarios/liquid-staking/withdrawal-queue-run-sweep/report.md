...
- **Adapter**: `/home/ailton/Work/riptide/riptide/fixtures/adapters/liquid-staking.toml`
- **Seed**: 1721008993
- **Ticks**: 24
- **Agents**: 20 (5× Steady staker, 5× Yield maximizer, 7× Panic exiter, 3× Arb redeemer)
- **Scenario**: baseline
- **Output**: `/home/ailton/Work/riptide/riptide/fixtures/scenarios/liquid-staking/withdrawal-queue-run-sweep`
...
| pool.lst_supply_avg | 120 |
| pool.lst_supply_max | 240 |
...
| pool.total_assets_avg | 120 |
| pool.total_assets_max | 240 |
...
| stake_account.lst_balance_avg | 6 |
| stake_account.lst_balance_max | 12 |
...
- T1: Arb redeemer (agent-019) — request_unstake → failed
- T1: Arb redeemer (agent-020) — request_unstake → failed
...
```sh
exec riptide run fixtures/scenarios/liquid-staking/withdrawal-queue-run-sweep/run-config.json --adapter fixtures/adapters/liquid-staking.toml
```
