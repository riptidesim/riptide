...
fn sizing_amount(policy: &Policy, agent: &Agent) -> f64 {
    match policy.position_sizing.strategy {
        crate::kernel::types::PositionSizingStrategy::Fixed => policy
            .position_sizing
            .params
            .get("amount")
            .copied()
            .unwrap_or(0.0),
        crate::kernel::types::PositionSizingStrategy::Proportional => {
            let percent = policy
                .position_sizing
                .params
                .get("percent")
...
mod tests {
    ...
    use super::*;
    use crate::kernel::{
        persona::{runtime::AgentObservation, state::Agent},
