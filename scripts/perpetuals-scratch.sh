#!/usr/bin/env bash
# perps persona scratch runner.
#
# Mirrors the `scripts/solend-whale-grid.sh` sidecar pattern:
...
BASE_ADAPTER="fixtures/adapters/perpetuals.toml"
...
awk '/^# === SIDECAR-CUT ===$/ { exit } { print }' "$BASE_ADAPTER" \
  ...
  > "$RUNTIME_ADAPTER"
echo "# --- Perps persona library (sidecar merge) ---" >> "$RUNTIME_ADAPTER"
for pid in "${PERSONA_IDS[@]}"; do
