#!/usr/bin/env bash
# Sprint 6 Phase 2 T13 — AMM persona scratch runner.
#
# Mirrors the Sprint 5 `scripts/sprint5-perps-scratch.sh` sidecar pattern:
...
#
# This is a throwaway determinism gate — T13 ships the persona files
# and this script as the bundle's end-to-end shape check; it is NOT
# the cold-chain validation story (that's T17).
...
awk '/^# === SIDECAR-CUT ===$/ { exit } { print }' "$BASE_ADAPTER" \
  ...
  > "$RUNTIME_ADAPTER"
echo "# --- AMM persona library (T13 sidecar merge) ---" >> "$RUNTIME_ADAPTER"
for pid in "${PERSONA_IDS[@]}"; do
