...
then `riptide-harness` when setup code is needed, then
`riptide-scenarios`:
...
2. **This skill** fills in the TODO blocks of the scaffolded adapter
   ...
   exists), runs `riptide lint`, and runs `riptide adapt` only when
   an adapter-only smoke can boot without project-specific setup.
3. `riptide-harness` creates or repairs the Rust setup crate for
   account bytes, PDAs, SPL accounts, state packs, and sibling CPI
   programs the adapter needs before tick 0. Prefer harness code over
   adding protocol-specific setup to Riptide core.
4. `riptide-scenarios` adds further `.riptide/scenarios/**`
   run-configs in user repos, or fixture-style scenario directories
   in the monorepo. Generic-runtime personas stay inline in the
...
- **Rust harness setup** — `.riptide/harness/src/main.rs` generated or
  repaired by the `riptide-harness` skill. Use it for
  protocol-specific account bytes, SPL mints/vaults, PDAs, sibling
  programs, and external state; do not encode those layouts into
  adapter TOML unless they are generic account declarations or decoder
  mappings.
...
Fix all `FAIL` findings and all coverage warnings caused by unmapped
IDL instructions or fields that are not acknowledged in
`[lineage].unsupported_fields`.

Then decide whether an adapter-only smoke is meaningful. Run it for
adapters whose declared/default account bytes can boot the mapped
action:
...
Skip or treat `riptide adapt` failure as "harness required" when the
adapter depends on nonzero SPL accounts, PDAs with concrete data,
oracle accounts, sibling programs, state packs, or external decoder
accounts initialized before tick 0. Common adapter-only failures in
that case include zero decoded balances, Anchor constraint errors
from uninitialized accounts, invariant fires against zeroed setup
accounts, and "no observation delta" because the instruction could
not execute. Do not iterate the adapter indefinitely on those; finish
the static adapter and proceed to `riptide-harness`.

### 8. Report back

- **lint PASS + adapt PASS** → show the adapter path, call out any
  remaining `# TODO:` markers, and tell the user they can run
  `riptide-harness` only if richer setup is needed.
- **lint PASS + harness-required adapt failure/skip** → show the
  adapter path, summarize why adapter-only smoke cannot boot, and
  proceed to `riptide-harness` instead of rewriting a structurally
  valid adapter.
- **lint FAIL/WARN** → fix the named adapter coverage or schema issue
  before moving on.
- **adapt exit 1 for a non-setup reason** → show the engine stderr
  tail and iterate on the adapter bindings.
- **adapt exit 2** → the TOML failed to parse or failed the Zod
  validator. Read the specific error, fix the one field it names, and
  rerun the command. Do not hand the user a malformed adapter.
