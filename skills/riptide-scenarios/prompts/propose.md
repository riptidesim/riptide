...
In fixture mode, `personas` is the explicit agent population mix and
references persona ids declared in that scenario's fixture policy
catalog. This is how the engine knows how to populate the agent pool
with the mix. Do not copy this fixture-mode shape into user repos;
there, personas stay inline in the adapter and the run-config
`personas` array may be empty or contain adapter-inline persona ids.
